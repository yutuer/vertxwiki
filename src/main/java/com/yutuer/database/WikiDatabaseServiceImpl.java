...
import com.yutuer.database.WikiDatabaseVerticle.SqlQuery;
...
public class WikiDatabaseServiceImpl implements WikiDatabaseService
{
	...
	WikiDatabaseServiceImpl(JDBCClient client, HashMap<SqlQuery, String> sqlQueries, Handler<AsyncResult<WikiDatabaseService>> readyHandler)
	{
	...
	public WikiDatabaseService fetchAllPages(Handler<AsyncResult<JsonArray>> resultHandler)
	{
		...
		dbClient.query(
				sqlQueries.get(SqlQuery.ALL_PAGES),
				res ->
				{
					if (res.succeeded())
					{
						JsonArray pages = new JsonArray(res.result().getResults().stream().map(json -> json.getString(0)).sorted()
